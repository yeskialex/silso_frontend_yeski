...
class _CommunityFindPageState extends State<CommunityFindPage> {
  ...
  Future<void> _loadAllCommunities() async {
    try {
      ...
      // For guests: no joined communities, for authenticated users: get their joined communities
      final joinedCommunities = _authService.isGuest 
          ? <Community>[]
          : await _communityService.getMyCommunities();
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          GestureDetector(
            onTap: isJoined ? null : () async {
              // Check if user is guest before allowing join
              if (_authService.isGuest) {
                await GuestLoginPrompt.show(context);
                return;
              }
              _joinCommunityAndShowDialog(community);
            },
            child: Container(
              width: 120,
              decoration: BoxDecoration(
