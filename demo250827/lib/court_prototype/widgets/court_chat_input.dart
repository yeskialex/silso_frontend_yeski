...
void _showCaseCardDialog() {
  ...
  Widget _buildChatTypeSwitcher(double switcherIconSize, Color coloricon) {
    // 선택된 타입에 따라 아이콘과 색상을 결정합니다.
    final IconData iconData = Icons.sync;  
    final Color iconColor =  coloricon; 
  ...
  Widget build(BuildContext context) {
    ...
    final Color borderColor = _selectedMessageType == ChatMessageType.notGuilty
    ...
    return GestureDetector(
      ...
      child: Column(
        ...
        children: [
Row(
  crossAxisAlignment: CrossAxisAlignment.center, // 이 속성은 이미 잘 사용하고 계시네요!
  children: [
    // Switch 버튼과 TextField 사이의 간격 추가
    const SizedBox(width: 12),

    // side select switch button
    // SizedBox로 높이를 명시하여 TextField와 높이 맞추기
    SizedBox(
      height: 48, // TextField의 높이와 맞추기 위해 값을 조정합니다.
      child: _buildChatTypeSwitcher(switcherIconSize, borderColor),
    ),
      ...
      child: Padding(
        ...
        child: TextField(
          ...
          decoration: InputDecoration(
            ...
            // 보더가 잘 보이도록 border 속성들을 수정
            enabledBorder: OutlineInputBorder(
              borderRadius: BorderRadius.circular(24),
              borderSide: BorderSide(
                color: borderColor,
                width: 1.0, // 보더 두께를 3.0으로 줄입니다.
              ),
            ),
    ...
    // TextField와 전송 버튼 사이의 간격 추가
    const SizedBox(width: 12),

    // Switch between send and docs icon based on keyboard visibility
    SizedBox(
      height: 48, // TextField와 높이 맞추기 위해 값을 조정합니다.
      width: 48, // 버튼의 너비도 명시하여 정사각형으로 만듭니다.
      child: Material(
        color: Colors.transparent,
        child: InkWell(
          onTap: widget.isEnabled ? _handleIconTap : null,
          borderRadius: BorderRadius.circular(24), // 버튼 모양에 맞게 조절
          child: AnimatedContainer(
            ...
            child: AnimatedSwitcher(
              duration: const Duration(milliseconds: 200),
              child: Icon(
                _hasText ? Icons.send : Icons.document_scanner,
