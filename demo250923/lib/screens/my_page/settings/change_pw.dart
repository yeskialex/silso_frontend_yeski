...
class _ChangePasswordPageState extends State<ChangePasswordPage> {
  ...
  Future<void> _changePassword() async {
    ...
    try {
      // Use AuthService to change password in Firebase
  ...
  String _getErrorMessage(String error) {
    if (error.contains('wrong-password') || error.contains('Wrong password')) {
      ...
      return '보안상 재로그인이 필요합니다. 로그아웃 후 다시 로그인해주세요.';
    } else if (error.contains('weak-password')) {
      return '새 비밀번호가 너무 약합니다. 더 강한 비밀번호를 사용해주세요.';
    } else if (error.contains('too-many-requests')) {
      return '너무 많은 시도가 있었습니다. 잠시 후 다시 시도해주세요.';
    } else if (error.contains('network-request-failed')) {
      return '네트워크 연결을 확인하고 다시 시도해주세요.';
    } else if (error.contains('No user is currently signed in')) {
