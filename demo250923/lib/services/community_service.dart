...
class CommunityService {
  ...
  // Step B: Verify phone number (no authentication required)
    ...
    try {
      await _auth.verifyPhoneNumber(
        ...
        verificationCompleted: (PhoneAuthCredential credential) async {
          // Phone verification completed automatically - just store the credential
  ...
  // Step B: Verify SMS code (no authentication required)
  Future<PhoneAuthCredential> verifySMSCode({
    ...
    try {
      ...
      // Return the credential instead of linking it immediately
      return credential;
    } catch (e) {
  ...
  Future<void> togglePostLike(String postId) async {
    try {
      ...
      await _firestore.runTransaction((transaction) async {
        ...
        // Initialize like fields if they don't exist (for legacy posts)
        if (!data.containsKey('likedBy') || !data.containsKey('likeCount')) {
          await _migrateLegacyPostToIncludeLikes(postId);
          // Re-fetch the document after migration
          final updatedDoc = await transaction.get(postRef);
  ...
  Future<void> _migrateLegacyPostToIncludeLikes(String postId) async {
    try {
      ...
      debugPrint('Error migrating legacy post: $e');
    }
  }
  ...
  Future<void> migrateAllPostsToIncludeLikes() async {
    try {
      ...
      for (QueryDocumentSnapshot doc in postsSnapshot.docs) {
        ...
        // Only migrate posts that don't have like fields
        if (!data.containsKey('likedBy') || !data.containsKey('likeCount')) {
      ...
      debugPrint('Successfully migrated posts to include like fields');
    } catch (e) {
