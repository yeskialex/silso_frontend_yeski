...
class TodayQuestionService {
  ...
  Future<String> createTodayQuestion(CreateTodayQuestionRequest request) async {
    // Don't allow writes in guest mode
    final authService = AuthService();
    if (authService.isGuest) {
      throw 'Guest users cannot create content';
    }
  ...
  Future<void> _createDefaultQuestion() async {
    // Don't create default content in guest mode
    final authService = AuthService();
    if (authService.isGuest) {
      debugPrint('Guest mode: skipping default question creation');
      return;
    }
